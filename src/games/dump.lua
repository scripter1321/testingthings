            ...
            for _,v in pairs(workspace.ActivePlots:GetChildren()) do
                if v.Name ~= "Plot" and v.Name ~= tostring(plr.UserId) then
                    ...
                    for i, item in pairs(v.PlacedItems:GetChildren()) do
                        ...
                        local targetPos = item.PrimaryPart.Position
                        local distance = (plr.Character.HumanoidRootPart.Position - targetPos).Magnitude
                        local tweenTime = distance / 50 -- Adjust 50 to change speed (higher = faster)
                        ...
                        local plotPos = workspace.ActivePlots[tostring(plr.UserId)].TeleportPoint.Position
                        local returnDist = (plr.Character.HumanoidRootPart.Position - plotPos).Magnitude
                        local returnTime = returnDist / 50
